...
class WritePoem():
    def __init__(self, args):
        ...
        with tf.name_scope('evaluation'):
            self.model = CharRNNLM(is_training=False, w2v_model=self.w2v.model, vocab_size=w2v_vocab_size, infer=True,
                                   **params)
    ...
    def rhyme_verse(self):
        ...
        if len(parts) > 0:
            ...
            sample = self.model.sample_seq(self.sess, gen_len, start,
                                           sample_type=SampleType.weighted_sample, rhyme_ref=rhyme_ref_word,
                                           rhyme_idx=rhyme_seq)
            print(sample)
    ...
    def hide_words(self, given_text):
        ...
        gen_len = 20
        sample = self.model.sample_seq(self.sess, gen_len, start_text='[', sample_type=SampleType.select_given,
                                       given=givens[0])
        if not sample:
            return 'err occar!'
        ...
        if len(parts) > 0:
            ...
            sample = self.model.sample_seq(self.sess, gen_len, start,
                                           sample_type=SampleType.select_given, given=givens[1],
                                           rhyme_ref=rhyme_ref_word, rhyme_idx=rhyme_seq)
            print(sample)
            return WritePoem.assemble(sample)
    ...
    def cangtou(self, given_text):
        ...
        for i in range(4):
            ...
            if (i != 3):
                ...
                sample = self.model.sample_seq(self.sess, self.args.length, start,
                                               sample_type=SampleType.max_prob, rhyme_ref=rhyme_ref_word,
                                               rhyme_idx=rhyme_seq)
